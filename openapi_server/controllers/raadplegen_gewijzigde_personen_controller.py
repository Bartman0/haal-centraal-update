...
from ..models import GewijzigdePersonenHalCollectie, GewijzigdePersonenHalCollectionLinks, HalLink, Foutbericht, \
    BadRequestFoutbericht, InvalidParams

BRP_PERSONEN_URL = "https://benk-brp-{omgeving}.amsterdam.nl/api/v1/brp/personen/{burgerservicenummer}"
...
connection_pool = ConnectionPool(psycopg_connection_info.render_as_string(hide_password=False), max_size=10)
...
def get_gewijzigde_personen(token_info, vanaf=None):  # noqa: E501
    ...
    if vanaf is None:
        return BadRequestFoutbericht(code="paramsRequired",
                                     ...
                                     invalid_params=InvalidParams(type="https://vng.nl/realisatie/api/validaties/datum",
                                         name="vanaf", code="date", reason="Waarde is geen geldige datum")).to_dict(), HTTPStatus.BAD_REQUEST
    afnemer_code = token_info['sub']
    headers = _get_default_headers()
    db, volgindicaties_db = get_volgindicaties_db()
    today = date.today()
    bsn_selectie = db.session.execute(db.select(volgindicaties_db.burgerservicenummer)
                              .where(volgindicaties_db.afnemer_code == afnemer_code)
                              .where(volgindicaties_db.begindatum <= today)
                              .where(volgindicaties_db.einddatum > today)).scalars().all()
    ...
    with connection_pool.connection() as connection:
        ...
        collectie = GewijzigdePersonenHalCollectie(
                                ...
                                _self=HalLink(href=BRP_WIJZIGINGEN_URL, templated=True, title="BRP API Personen"),
                                ingeschreven_persoon=HalLink(href=BRP_PERSONEN_URL, templated=True, title="BRP API Personen")
                            ),
